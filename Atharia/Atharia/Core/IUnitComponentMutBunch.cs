...
public class IUnitComponentMutBunch {
  ...
  public void CheckForNullViolations(List<string> violations) {
    ...
    if (!root.EvolvifyAICapabilityUCMutSetExists(membersEvolvifyAICapabilityUCMutSet.id)) {
      violations.Add("Null constraint violated! IUnitComponentMutBunch#" + id + ".membersEvolvifyAICapabilityUCMutSet");
    }
    ...
    if (!root.OnFireUCMutSetExists(membersOnFireUCMutSet.id)) {
      violations.Add("Null constraint violated! IUnitComponentMutBunch#" + id + ".membersOnFireUCMutSet");
    }
    ...
    if (!root.ExplosionRodMutSetExists(membersExplosionRodMutSet.id)) {
      violations.Add("Null constraint violated! IUnitComponentMutBunch#" + id + ".membersExplosionRodMutSet");
    }

    if (!root.BlazeRodMutSetExists(membersBlazeRodMutSet.id)) {
      violations.Add("Null constraint violated! IUnitComponentMutBunch#" + id + ".membersBlazeRodMutSet");
    }
  ...
  public void FindReachableObjects(SortedSet<int> foundIds) {
    ...
    if (root.EvolvifyAICapabilityUCMutSetExists(membersEvolvifyAICapabilityUCMutSet.id)) {
      membersEvolvifyAICapabilityUCMutSet.FindReachableObjects(foundIds);
    }
    if (root.TimeCloneAICapabilityUCMutSetExists(membersTimeCloneAICapabilityUCMutSet.id)) {
    ...
    if (root.OnFireUCMutSetExists(membersOnFireUCMutSet.id)) {
      membersOnFireUCMutSet.FindReachableObjects(foundIds);
    }
    if (root.AttackAICapabilityUCMutSetExists(membersAttackAICapabilityUCMutSet.id)) {
    ...
    if (root.ExplosionRodMutSetExists(membersExplosionRodMutSet.id)) {
      membersExplosionRodMutSet.FindReachableObjects(foundIds);
    }
    if (root.BlazeRodMutSetExists(membersBlazeRodMutSet.id)) {
      membersBlazeRodMutSet.FindReachableObjects(foundIds);
    }
    if (root.BlastRodMutSetExists(membersBlastRodMutSet.id)) {
  ...
  public EvolvifyAICapabilityUCMutSet membersEvolvifyAICapabilityUCMutSet {

    get {
      if (root == null) {
        throw new Exception("Tried to get member membersEvolvifyAICapabilityUCMutSet of null!");
      }
      return new EvolvifyAICapabilityUCMutSet(root, incarnation.membersEvolvifyAICapabilityUCMutSet);
    }
                       }
  ...
  public OnFireUCMutSet membersOnFireUCMutSet {

    get {
      if (root == null) {
        throw new Exception("Tried to get member membersOnFireUCMutSet of null!");
      }
      return new OnFireUCMutSet(root, incarnation.membersOnFireUCMutSet);
    }
                       }
  ...
  public ExplosionRodMutSet membersExplosionRodMutSet {

    get {
      if (root == null) {
        throw new Exception("Tried to get member membersExplosionRodMutSet of null!");
      }
      return new ExplosionRodMutSet(root, incarnation.membersExplosionRodMutSet);
    }
                       }
  public BlazeRodMutSet membersBlazeRodMutSet {

    get {
      if (root == null) {
        throw new Exception("Tried to get member membersBlazeRodMutSet of null!");
      }
      return new BlazeRodMutSet(root, incarnation.membersBlazeRodMutSet);
    }
                       }
...
,
      root.EffectEvolvifyAICapabilityUCMutSetCreate()
,
...
,
      root.EffectOnFireUCMutSetCreate()
,
...
,
      root.EffectExplosionRodMutSetCreate()
,
      root.EffectBlazeRodMutSetCreate()
,
...
,
  ...
  public void Add(IUnitComponent elementI) {
    ...
    // Can optimize, check the type of element directly somehow
    if (root.EvolvifyAICapabilityUCExists(elementI.id)) {
      this.membersEvolvifyAICapabilityUCMutSet.Add(root.GetEvolvifyAICapabilityUC(elementI.id));
      return;
    }
    ...
    if (root.OnFireUCExists(elementI.id)) {
      this.membersOnFireUCMutSet.Add(root.GetOnFireUC(elementI.id));
      return;
    }
    ...
    if (root.ExplosionRodExists(elementI.id)) {
      this.membersExplosionRodMutSet.Add(root.GetExplosionRod(elementI.id));
      return;
    }

    // Can optimize, check the type of element directly somehow
    if (root.BlazeRodExists(elementI.id)) {
      this.membersBlazeRodMutSet.Add(root.GetBlazeRod(elementI.id));
      return;
    }
  ...
  public void Remove(IUnitComponent elementI) {
    ...
    // Can optimize, check the type of element directly somehow
    if (root.EvolvifyAICapabilityUCExists(elementI.id)) {
      this.membersEvolvifyAICapabilityUCMutSet.Remove(root.GetEvolvifyAICapabilityUC(elementI.id));
      return;
    }
    ...
    // Can optimize, check the type of element directly somehow
    if (root.OnFireUCExists(elementI.id)) {
      this.membersOnFireUCMutSet.Remove(root.GetOnFireUC(elementI.id));
      return;
    }
    ...
    // Can optimize, check the type of element directly somehow
    if (root.ExplosionRodExists(elementI.id)) {
      this.membersExplosionRodMutSet.Remove(root.GetExplosionRod(elementI.id));
      return;
    }

    // Can optimize, check the type of element directly somehow
    if (root.BlazeRodExists(elementI.id)) {
      this.membersBlazeRodMutSet.Remove(root.GetBlazeRod(elementI.id));
      return;
    }
  ...
  public void Clear() {
    ...
    this.membersExplosionRodMutSet.Clear();
  ...
  public int Count {
    get {
      return
        ...
        this.membersExplosionRodMutSet.Count +
  ...
  public void Destruct() {
    ...
    var tempMembersEvolvifyAICapabilityUCMutSet = this.membersEvolvifyAICapabilityUCMutSet;
    ...
    var tempMembersOnFireUCMutSet = this.membersOnFireUCMutSet;
    ...
    var tempMembersExplosionRodMutSet = this.membersExplosionRodMutSet;
    ...
    tempMembersExplosionRodMutSet.Destruct();
  ...
  public IEnumerator<IUnitComponent> GetEnumerator() {
    ...
    foreach (var element in this.membersEvolvifyAICapabilityUCMutSet) {
      yield return new EvolvifyAICapabilityUCAsIUnitComponent(element);
    }
    ...
    foreach (var element in this.membersOnFireUCMutSet) {
      yield return new OnFireUCAsIUnitComponent(element);
    }
    ...
    foreach (var element in this.membersExplosionRodMutSet) {
      yield return new ExplosionRodAsIUnitComponent(element);
    }
    foreach (var element in this.membersBlazeRodMutSet) {
      yield return new BlazeRodAsIUnitComponent(element);
    }
    ...
    public List<EvolvifyAICapabilityUC> GetAllEvolvifyAICapabilityUC() {
      var result = new List<EvolvifyAICapabilityUC>();
      foreach (var thing in this.membersEvolvifyAICapabilityUCMutSet) {
        result.Add(thing);
      }
      return result;
    }
    public List<EvolvifyAICapabilityUC> ClearAllEvolvifyAICapabilityUC() {
      var result = new List<EvolvifyAICapabilityUC>();
      this.membersEvolvifyAICapabilityUCMutSet.Clear();
      return result;
    }
    public EvolvifyAICapabilityUC GetOnlyEvolvifyAICapabilityUCOrNull() {
      var result = GetAllEvolvifyAICapabilityUC();
      Asserts.Assert(result.Count <= 1);
      if (result.Count > 0) {
        return result[0];
      } else {
        return EvolvifyAICapabilityUC.Null;
      }
    }
    ...
    public List<OnFireUC> GetAllOnFireUC() {
      ...
      foreach (var thing in this.membersOnFireUCMutSet) {
        result.Add(thing);
      }
      return result;
    }
    ...
    public List<ExplosionRod> GetAllExplosionRod() {
      ...
      foreach (var thing in this.membersExplosionRodMutSet) {
        result.Add(thing);
      }
      return result;
    }
    ...
    public List<BlazeRod> GetAllBlazeRod() {
      ...
      foreach (var thing in this.membersBlazeRodMutSet) {
        result.Add(thing);
      }
      return result;
    }
    ...
    public IImpulsePostReactor GetOnlyIImpulsePostReactorOrNull() {
      ...
      foreach (var obj in this.membersExplosionRodMutSet) {
        result.Add(
            new ExplosionRodAsICloneableUC(obj));
      }
      foreach (var obj in this.membersBlazeRodMutSet) {
        result.Add(
            new BlazeRodAsICloneableUC(obj));
      }
    ...
    public IMovementTimeFactorUC GetOnlyIMovementTimeFactorUCOrNull() {
      ...
      foreach (var obj in this.membersEvolvifyAICapabilityUCMutSet) {
        result.Add(
            new EvolvifyAICapabilityUCAsIAICapabilityUC(obj));
      }
      foreach (var obj in this.membersBideAICapabilityUCMutSet) {
        result.Add(
    ...
    public IPickUpReactorItem GetOnlyIPickUpReactorItemOrNull() {
      ...
      foreach (var obj in this.membersOnFireUCMutSet) {
        result.Add(
            new OnFireUCAsIPreActingUC(obj));
      }
      foreach (var obj in this.membersDefyingUCMutSet) {
        result.Add(
    ...
    public IImmediatelyUseItem GetOnlyIImmediatelyUseItemOrNull() {
      ...
      foreach (var obj in this.membersExplosionRodMutSet) {
        result.Add(
            new ExplosionRodAsIItem(obj));
      }
      foreach (var obj in this.membersBlazeRodMutSet) {
        result.Add(
            new BlazeRodAsIItem(obj));
      }
      foreach (var obj in this.membersBlastRodMutSet) {
        result.Add(
