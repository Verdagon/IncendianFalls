...
namespace ConsoleDriveyThing {
  class RandomPlayer {
    static string HandleDirection(Superstructure ss, Game game, Location destination) {
      ...
      foreach (var unit in game.level.units) {
        if (unit.location == destination) {
          var(_, res) = ss.RequestAttack(game.id, unit.id);
          return res;
        }
      }

      var(_, result) = ss.RequestMove(game.id, destination);
      return result;
    }
    ...
    public static void Play() {
      ...
      //    if (action == 0) {
      //      var(_, defendResult) = ss.RequestDefy(game.id);
      //      if (defendResult == "") {
      //        ss.RequestResume(game.id);
      //      } else { 
      ...
      //        var(_, timeShiftError) = ss.RequestTimeShift(game.id);
